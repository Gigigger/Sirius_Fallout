warp-point-evacshuttle = Эвакуационный шаттл
warp-point-shuttle = Трансфер
warp-point-debris = Космический мусор
warp-point-ruin = Космические руины

warp-point-bridge = Мост
warp-point-vault = Сейф

warp-point-sec = Безопасность
warp-point-perma = Перма
warp-point-detective = Детектив
warp-point-court = Зал суда

warp-point-medical = Медицинский
warp-point-morgue = Морг

warp-point-epistemics = Эпистемика

warp-point-logistics = Логистика
warp-point-salvage = Спасение

warp-point-engineering = Инженерное дело
warp-point-singularity = Сингулярность
warp-point-atmospherics = Атмосфера

warp-point-hop = Прыгать
warp-point-kitchen = Кухня
warp-point-bar = Бар
warp-point-botany = Ботаника
warp-point-janitor = Дворник
warp-point-reporter = Репортер
warp-point-lawyer = Адвокат

warp-point-ai = ИИ
warp-point-arrivals = Прибытие
warp-point-evac = Эвак
warp-point-cryo = Крио
warp-point-chapel = Часовня
warp-point-library = Библиотека
warp-point-dorms = Общежития
warp-point-disposals = Выбытия