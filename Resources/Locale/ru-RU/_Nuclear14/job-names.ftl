# Brotherhood of Steel
job-name-bos-mid-paladin-comm = Паладин-командор Братства
job-description-bos-mid-paladin-comm = Возглавьте боевое крыло отделения Братства и руководите полевыми операциями.
job-name-bos-mid-paladin = Старший паладин Братства
job-description-bos-mid-paladin = Служите ветераном полевого командира и командиром тяжелой пехоты Братства.
job-name-bos-mid-knight = Рыцарь-сержант Братства
job-description-bos-mid-knight = Возглавьте патрули Братства, поддерживайте дисциплину и защищайте восстановленные технологии.
job-name-bos-mid-scribe = Проктор Братства
job-description-bos-mid-scribe = Координируйте исследования, каталогизируйте технологии и консультируйте командный состав отделения.
job-name-bos-mid-squire = Оруженосец Братства
job-description-bos-mid-squire = Тренируйтесь под руководством ветеранов Братства и докажите, что вы готовы к более высоким обязанностям.

#Misfits Change /Comment-out/: Washington BoS job names removed — will create new prototypes.
## Washington Brotherhood
#job-name-bos-washington-commander = Brotherhood Head Knight
#job-description-bos-washington-commander = Oversee the Brotherhood's knightly orders, logistics, and battlefield readiness.
#job-name-bos-washington-initiate = Brotherhood Initiate
#job-description-bos-washington-initiate = Learn the Brotherhood's code, follow orders, and earn your place in the chapter.
#job-name-bos-washington-knight = Brotherhood Knight
#job-description-bos-washington-knight = Protect the chapter, escort missions, and recover technology in the field.
#job-name-bos-washington-paladin = Brotherhood Paladin
#job-description-bos-washington-paladin = Fight as the Brotherhood's frontline heavy infantry and defend the chapter's interests.
#job-name-bos-washington-scribe = Brotherhood Scribe
#job-description-bos-washington-scribe = Support the chapter with research, medicine, and technical expertise.

# Caravan
job-name-caravan-leader = Лидер Каравана
job-description-caravan-leader = Координируйте деятельность караванной компании в этом районе и заключайте торговые сделки с местными жителями.
job-name-caravan-trader = Торговец Каравана
job-description-caravan-trader = Проводите торговые миссии и путешествуйте по караванным маршрутам, чтобы приносить прибыль компании.
job-name-caravan-guard = Охранник Каравана
job-description-caravan-guard = Защитите торговцев караванной компании, их скот и инвентарь.

# NCR
job-name-ncr-recruit = Штрафник НКР
job-description-ncr-recruit = Будь-то бывший зек, рейдер или несчастный солдат - вы отныне Штрафник НКР, а ваша задача исправиться и возобновить честь мундира, если вы конечно не надумали висеть на плахе
job-name-ncr-cadet = Штрафник НКР
job-description-ncr-cadet = Будь-то бывший зек, рейдер или несчастный солдат - вы отныне Штрафник НКР, а ваша задача исправиться и возобновить честь мундира, если вы конечно не надумали висеть на плахе
job-name-ncr-doctor = Врач НКР
job-description-ncr-doctor = Контролируйте уход за полем боя, поддерживайте отряд живым и направляйте медицинскую помощь НКР.
job-name-ncr-medic = Медик НКР
job-description-ncr-medic = Лечите раненых сотрудников НКР в полевых условиях и продолжайте движение колонны.
job-name-ncr-first-sergeant = Сержант НКР первого класса
job-description-ncr-first-sergeant = Поддерживайте дисциплину, координируйте действия рядовых и сохраняйте целостность линии НКР под руководством капитана и лейтенанта.
job-name-ncr-staff-sergeant = Старший сержант НКР
job-description-ncr-staff-sergeant = Возглавьте войска НКР на поле боя и выполняйте приказы командного состава.
job-name-ncr-sergeant = Сержант НКР
job-description-ncr-sergeant = Возглавьте боевую группу, поддерживайте порядок и выполняйте планы старших сержантов и офицеров.
job-name-ncr-sgt = Сержант НКР
job-description-ncr-sgt = Возглавьте боевую группу, поддерживайте порядок и выполняйте планы старших сержантов и офицеров.
# #Misfits Removed - NCR Major locale moved to _Misfits/corvax-jobs.ftl (authoritative Misfits version)
# job-name-ncr-major = NCR Major
# job-description-ncr-major = Serve as the senior Command officer of the NCR detachment, answerable only to high command. Set operational priorities, adjudicate disputes between officers, and ensure the mission succeeds.
job-name-ncr-captain = Капитан НКР
job-description-ncr-captain = Командуйте отрядом, расставляйте приоритеты и отвечайте только перед высшим командованием НКР и миссией.
job-name-ncr-lt = Лейтенант НКР
job-description-ncr-lt = Выполняйте обязанности заместителя капитана, руководите сержантами и следите за бесперебойной работой отряда.
job-name-ncr-private-first-class = Пехотинец первого класса НКР
job-description-ncr-private-first-class = Действуйте как опытный линейный солдат и помогите укрепить ряды ниже корпуса унтер-офицеров.
job-name-ncr-private = НКР Пехотинец
job-description-ncr-private = Служите линейным солдатом НКР и выполняйте приказы своих сержантов и офицеров.
job-name-ncr-soldier = НКР Пехотинец
job-description-ncr-soldier = Служите линейным солдатом НКР и выполняйте приказы своих сержантов и офицеров.
job-name-ncr-engineer = Инженер НКР
job-description-ncr-engineer = Поддерживайте технику НКР, стройте оборону и поддерживайте боеготовность отряда.
job-name-ncr-corporal = Специалист НКР
job-description-ncr-corporal = Служите обученным специалистом — будь то в бою, инженерном деле или медицине — и закрепите линию под корпусом унтер-офицеров.
job-name-ncr-ws = Специалист НКР
job-description-ncr-ws = Служите обученным специалистом — будь то в бою, инженерном деле или медицине — и закрепите линию под корпусом унтер-офицеров.
# #Misfits Add - new locale keys for the Specialist rename
job-name-ncr-specialist = Специалист НКР
job-description-ncr-specialist = Служите обученным специалистом — будь то в бою, инженерном деле или медицине — и закрепите линию под корпусом унтер-офицеров.
job-name-ncr-ranger = Рейнджер НКР
job-description-ncr-ranger = Самые опытные и сильные солдаты входят в состав этой элитной группы Армии НКР, возглавляют войска или действуют самостоятельно по приказу Республики.
job-name-ncr-ranger-veteran = Ветеран рейнджеров НКР
job-description-ncr-ranger-veteran = Самые опытные и сильные солдаты входят в состав этой элитной группы Армии НКР, возглавляют войска или действуют самостоятельно по приказу Республики.

# Rangers
job-name-ranger-patrol = Патрульный рейнджер
job-description-ranger-patrol = Разведайте впереди, патрулируйте границу и докажите, что вы принадлежите к рейнджерам.
job-name-ranger-recruit = Патрульный рейнджер
job-description-ranger-recruit = Разведайте впереди, патрулируйте границу и докажите, что вы принадлежите к рейнджерам.
job-name-ranger-field = Полевой рейнджер
job-description-ranger-field = Действуйте самостоятельно или небольшими группами, соблюдайте законы рейнджеров и защищайте границу.
job-name-ranger-ranking = Полевой рейнджер
job-description-ranger-ranking = Действуйте самостоятельно или небольшими группами, соблюдайте законы рейнджеров и защищайте границу.
job-name-ranger-chief = Главный рейнджер пустыни
job-description-ranger-chief = Как главный рейнджер пустыни, вы руководите всеми операциями рейнджеров, обеспечивая безопасность и обучение своих рядов. Ваша миссия — создавать альянсы, руководить крупномасштабными миссиями и поддерживать наследие справедливости в пустоши.
job-name-ranger-veteran-n14 = Ветеран-рейнджер
job-description-ranger-veteran-n14 = Руководите операциями рейнджеров в регионе, выбирайте свои методы и отвечайте только своей миссии и своему кодексу.
job-name-ranger-veteran = Ветеран-рейнджер
job-description-ranger-veteran = Руководите операциями рейнджеров в регионе, выбирайте свои методы и отвечайте только своей миссии и своему кодексу.

# #Misfits Change - rename tribe role display names to Yuma variants without changing job IDs.
# Tribal
job-name-tribal = Сородич Племени
job-description-tribal = Любой ценой поддержите образ жизни племени Юма на пустошах. Безопасность, ремесло, домашняя жизнь — все это делаете вы.
job-name-tribal-elder = Старейшина Племени
job-description-tribal-elder = Ведите племя Юма через хорошие и плохие времена и сохраняйте свои убеждения.
job-name-tribal-farmer = Фермер Племени
job-description-tribal-farmer = Производите еду и товары для племени Юма, чтобы обеспечить себе жизнь.
job-name-tribal-shaman = Шаман Племени
job-description-tribal-shaman = Проводите духовные обряды и излечивайте раны Соплеменников.

# Vault
job-name-overseer = Смотритель Убежища
job-description-overseer = Следуйте целям «Волт-Тек» в управлении вашим Убежищем и убедитесь, что все идет по плану...
job-name-vault-doctor = Врач Убежища
job-description-vault-doctor = Поддерживайте здоровье и благополучие своих собратьев-жителей Убежища. При необходимости провести исследование.
job-name-vault-dweller = Резидент Убежища
job-description-vault-dweller = Основное население Убежища. Живите комфортной жизнью и тяните свой вес там, где это необходимо.
job-name-vault-engineer = Инженер Убежища
job-description-vault-engineer = Поддерживайте Убежище и все его жизненно важные системы, чтобы обеспечить комфортную и безопасную жизнь.
job-name-vault-security = Охранник Убежища
job-description-vault-security = Поддерживайте порядок в Убежище и докладывайте Смотрителю. На благо Убежища!

# Wasteland
# #Misfits Change - renamed for Town faction clarity
job-name-farmer = Городской фермер
job-description-farmer = Выращивайте урожай на пустоши для себя или для обмена с другими.
job-name-waste-musician = Музыкант
job-description-waste-musician = Эта роль будет удалена из игры, пожалуйста, установите для всех своих персонажей значение «НИКОГДА»!
job-name-scavenger = Мусорщик
job-description-scavenger = Эта роль будет удалена из игры, пожалуйста, установите для всех своих персонажей значение «НИКОГДА»!
job-name-wastelander = Выходец с пустоши
job-description-wastelander = Выживайте в пустоши и прокладывайте свой собственный путь. В одиночку там тяжело, подумай о поиске других.

# Town
job-name-townsperson = Горожанин
job-description-townsperson = Член послевоенного общества. Следуйте городским правилам или попытайтесь баллотироваться на пост мэра, чтобы изменить их.
job-name-towndoctor = Городской доктор
job-description-towndoctor = Продавайте свои медицинские знания как услугу. Лечите раненых за деньги и управляйте врачебной практикой в ​​городе.
job-name-townmechanic = Городской механик
job-description-townmechanic = Поддерживайте городские службы и инфраструктуру и делайте все возможное для других за деньги.
job-name-townshopkeeper = Городской лавочник
job-description-townshopkeeper = Открывайте магазин в городе, организуйте торговые партнерства и продавайте свои товары странникам по пустошам. Остерегайтесь налоговиков.
job-name-towndeputy = Стража ХанТауна
job-description-towndeputy = Сотрудник правоохранительных органов города. Поддерживайте мир и обеспечивайте поддержку своему Пахану.
job-name-townsheriff = Глава Стражи ХанТауна
job-description-townsheriff = Поддерживайте закон и порядок в городе и поддерживайте Пахана в его правлении.
job-name-townmayor = Пахан ХанТауна
job-description-townmayor = Держите город в узде и правьте железной рукой. Установите налоги и законы и постарайтесь не быть убитым или захваченными.
job-name-townreporter = Городской репортер
job-description-townreporter = Берите интервью у людей, распространяйте новости по всему региону или просто устраивайте ток-шоу!
job-name-townbartender = Городской трактирщик
job-description-townbartender = Управляйте местным баром, кухней и комнатами, которые сдаются в аренду. Открой бизнес в городе!

# Followers
job-name-followers = Последователи Апокалипсиса
job-description-followers = Отбросьте дух Последователей еще глубже в пустоши. Относитесь к населению ради общего блага.

# Fun
job-name-survivor = Выживший
job-description-survivor = Выживайте так долго, как сможете! Это никогда не заканчивается.
job-name-zetan = Зетанский похититель
job-description-zetan = За материнский корабль! Исследуйте местное население для достижения своих целей.

# N14 Role Timers # Is this needed?
Survivor = Выживший
