# Factions
guide-entry-factions = Фракции и роли
guide-entry-brotherhoodofsteel = Братство Стали (БС)
guide-entry-caesarslegion = Легион Цезаря
guide-entry-caravancompany = Караванная компания
guide-entry-childrenofatom = Дети Атома
guide-entry-enclave = Анклав
guide-entry-followersoftheapocalypse = Последователи Апокалипсиса
guide-entry-minorfactions = Второстепенные фракции
guide-entry-ncr = НКР
guide-entry-townsfolk = Город
guide-entry-tribes = Племена
guide-entry-vaultdwellers = Сейф
guide-entry-wastelanders = Жители пустошей
guide-entry-zetan = Зетанцы

# Survival
guide-entry-survivalindex = Выживание
guide-entry-basics = Основы выживания

# Weapons and Ammo
guide-entry-weaponsammo = Оружие и боеприпасы
guide-entry-rangedweapons = Оружие дальнего боя
guide-entry-bigguns = Большие пушки
guide-entry-laser = Энергетическое оружие
guide-entry-plasma = Плазменное оружие
guide-entry-smallguns = Маленькие пушки
guide-entry-meleeweapons = Холодное оружие
guide-entry-onehanded = Одноручное оружие
guide-entry-twohanded = Двуручное

# Crafting
guide-entry-crafting = Крафт
guide-entry-bartending = Напитки и другие жидкости
# Misfits Fix - guide-entry-botany already defined in guidebook/guides.ftl; removed duplicate
guide-entry-cooking = Кулинария
# Misfits Fix - guide-entry-crafting-sys already defined in guidebook/guides.ftl; removed duplicate
guide-entry-materials = Материалы
# Misfits Fix - guide-entry-medicine already defined in guidebook/guides.ftl; removed duplicate